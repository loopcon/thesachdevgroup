...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('car-insurance-update') }}" method="POST" class="service-center-form" enctype="multipart/form-data" data-parsley-validate="">
                        @csrf
                        <div class="row">
                            <div class="col-md-4 mt-2">
                                ...
                                @if(isset($record->banner_image) && $record->banner_image)
                                    <img src="{{url('public/uploads/carInsurance/'.$record->banner_image)}}" width="100">
                                @endif  
                            ...
                            <div class="col-md-4">
                                <label for="insurance_form_title" class="form-label">Insurance Form Title</label>
                                <input type="text" class="form-control" id="insurance_form_title" name="insurance_form_title" value="{{isset($record->insurance_form_title) ? $record->insurance_form_title : old('insurance_form_title')}}">
                                @if ($errors->has('insurance_form_title')) <div class="text-danger">{{ $errors->first('insurance_form_title') }}</div>@endif
                            </div>

                            <div class="col-md-4">
                                <label for="insurance_form_title_color" class="form-label">Insurance Form Title Color</label>
                                <input type="text" id="insurance_form_title_color" class="form-control colorpicker" name="insurance_form_title_color" value="{{isset($record->insurance_form_title_color) ? $record->insurance_form_title_color : old('insurance_form_title_color')}}">
                            </div>

                            <div class="col-md-4">
                                <label for="insurance_form_title_font_size" class="form-label">Insurance Form Title Font Size</label>
                                <select class="form-control select2" name="insurance_form_title_font_size">
                                    <option value="">Select</option>
                                    @for($i=24; $i<=50; $i+=2)
                                        <option value="{{$i}}px" @if(isset($record->insurance_form_title_font_size) && $record->insurance_form_title_font_size == $i.'px'){{'selected'}}@endif>{{$i}}px</option>
                                    @endfor
                                </select>
                            </div>
                            ...
                            @php($fontfamily = fontFamily())
                            <div class="col-md-4 mt-2">
                                <label for="insurance_form_title_font_family" class="form-label">Insurance Form Title Font Family</label>
                                <select class="form-control select2" name="insurance_form_title_font_family">
                                    <option value="">Select</option>
                                    @foreach($fontfamily as $family)
                                        <option value="{{$family['key']}}" @if(isset($record->insurance_form_title_font_family) && $record->insurance_form_title_font_family == $family['key']){{'selected'}}@endif>{{$family['value']}}</option>
                                    @endforeach
                                </select>
                            </div>
